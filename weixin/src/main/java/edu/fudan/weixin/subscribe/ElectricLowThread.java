 ...
 *
 */
public class ElectricLowThread implements Runnable {

	private static Log log = LogFactory.getLog(ElectricLowThread.class);
	...
	public void run() {
		...
		while (cs.hasNext()) {
			DBObject bookobj=cs.next();
			String openid = String.valueOf(bookobj.get("openid"));
			Double threshold=(Double)bookobj.get("threshold");
						...
						if (ol != null && ol instanceof List) {
							...
							for (Object oo : ll) {
								if (oo != null && oo instanceof DBObject) {
									...
									if (balance != null&&Double.parseDouble(String.valueOf(balance))<threshold) {
										msg.put("first", "截止到"+dbo.get("elecdate")+"您的电量余额不足，为了您的正常用电请及时充值。");
										msg.put("keynote1",
												dbo.get("person_name") + "("
														+ dbo.get("username")
														+ ")");
