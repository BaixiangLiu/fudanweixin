		...
		if (CommonUtil.isEmpty(getSession().get("openid"))) {
			...
			try {
				...
				redir = "https://open.weixin.qq.com/connect/oauth2/authorize?appid="
						...
						+ "&redirect_uri="
						+ EncodeHelper
								.encode(conf.get("weixin.context")+"wxlogindo.act", "URL")
		...
		if (!CommonUtil.isEmpty(code)
			...
			try {
				...
				if (!CommonUtil.isEmpty(acctk)) {
					...
					c.save(obj);
					/*
					if(obj.get("binds")!=null &&obj.get("binds") instanceof List)
					{
						List<BasicDBObject> ls=new ArrayList<BasicDBObject>();
						for(DBObject ob:(List<DBObject>)obj.get("binds"))
						{
							if(!CommonUtil.isEmpty(ob)){							
							BasicDBObject bdo=new BasicDBObject();
							bdo.put("uisid", ob.get("uisid"));
							bdo.put("username",ob.get("username"));
							bdo.put("usertype", ob.get("usertype"));
							ls.add(bdo);
							}
						}
