...
$(
	function(){		
		$.getJSON("resource/elec.act",{},function(resp){
			...
			if(resp.result.errcode==0)
			{
				...
				for(var i=0;i<resp.list.length;i++)
				{
					...
					if(data.error==null)
					{
						...
						htm+="<tr><td>房间</td><td>"+data.list[j].school_space+data.list[j].room+"</td></tr>";
						htm+="<tr><td>已用电量</td><td>"+data.list[j].usedamp+"度</td></tr>";
						htm+="<tr><td>可用电量 </td><td>"+data.list[j].canuse+"度</td></tr>";		
						htm+="<tr><td>昨日用电量</td><td>"+data.list[j].useelec+"度</td></tr>";
						htm+="<tr><td>抄表时间</td><td>"+data.list[j].elecdate+"</td></tr></table>";
						}
						}
					htm+="</div></div>";	
				}	
