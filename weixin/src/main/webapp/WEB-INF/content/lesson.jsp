...
<script type="text/javascript">
	$(function() {
		...
		var ty = 2014;
		for (var i = 0; i < 10; i++)
			ys.options.add(new Option(((ty  - i) + "-" + (ty+1 - i)), (ty - i)));
		
		$.getJSON("resource/schoolterm.act",{},function(resp){
			if (resp.result.errcode == 0) {
				$("#pcontent").html(
						"<div class='alert alert-success'>现在是"+resp.list.termname+"学期的第"+resp.list.week+"周</div>");
			} else {
				$("#pcontent").html(
		...
		$
				.getJSON(
						...
						function(resp) {
							var htm="";
							if (resp.result.errcode == 0) {
								if (resp.list.length==0||resp.list[0].list.error == "access_denied")
									htm = "<div class='alert alert-warning'>尚未完成绑定操作，请重新对此UIS账号进行绑定。</div>";
								if (resp.list[0].list.error == "invalid_scope")
									htm = "<div class='alert alert-warning'>未对此UIS账号的课程项目授权，请发送语音或文字消息【修改授权】并按提示进行操作</div>";
								if (resp.list[0].list.error == null) {
									if (resp.list[0].list.length == 0)
										htm = "<div class='alert alert-warning'>所查学期无课程信息</div>";
									else {
										htm = "<div class='table-responsive'><table class='table table-striped table-condensed'><tr><th colspan='5'>"+resp.list[0].list[0].term+"</th></tr><tr><th>课程代码</th><th>课程名称</th><th>课程类型</th><th>主讲老师</th><th>学分</th></tr> ";
										for (var i = 0; i < resp.list[0].list.length; i++) {
											...
											htm += "<tr><td>" + data.course_id
													...
													+ "</td>";
													htm += "<td>" + data.course_type
													+ "</td>";
													htm += "<td>" + data.teacher
													+ "</td>";
											htm += "<td>" + data.credit
													+ "</td></tr>";
											htm+="<tr><th>上课时间地点</th><td colspan='4'>"+data.sksj+"</td></tr>";		
										}
										htm += "</table></div>";
									}
								}
				...
				<div class=" panel panel-primary">
					<div class="panel-heading">
						<form class="form-inline" role="form" onsubmit="return subform()"
							...
							<button type="submit" class="btn btn-success btn-sm">课程查询</button>
